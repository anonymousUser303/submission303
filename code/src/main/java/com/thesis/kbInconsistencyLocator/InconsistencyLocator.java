...
/**
 *
 * @author anonymoussubmittor@gmail.com
 */
...

{
    ...
    private static List<Integer> subgraphSize = new ArrayList<>();
    public static double avgSubgraph = 0;
...
//            subGraph = GraphExtract.extractExtend(tripleItem, hdt, maxValue);
    ...
    private static Set<Set<OWLAxiom>>  WriteInconsistencySubGraph(HDT hdt, String tripleItem) throws Exception{
        ...
        subgraphSize.add( subGraph.size());
    ...
    private static Set<String> RetrieveAll(HDT hdt) throws Exception{
        ...
        while (it.hasNext())
        {
            ...
            subGraph.add( t.asNtriple().toString() );
        }
        return subGraph;
}
...
//        Iterator it = hdt.getDictionary().getSubjects().getSortedEntries();
        IteratorTripleString it = hdt.search("","","");
        System.out.println("HDT size : "+ hdt.getTriples().getNumberOfElements());
        long size = hdt.getDictionary().getNsubjects();
    ...
    public static void main( String[] args ) throws Exception {
        ...
        avgSubgraph = calcAverage(subgraphSize);
        fileWriter.write(("Stopped").getBytes());

    }

    private static double calcAverage(List<Integer> arr){
        double total = 0;
